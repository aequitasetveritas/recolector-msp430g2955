/* msprf24.c
 * Funciones del MSP430 para el control del tranceptor RF nRF24L01+ RF de
 ...
 */
/* Variables privadas de la libreria */
uint8_t rf_feature;  // Usada para registrar que caracteristicas (features) han sido utilizadas.

/* CE (Chip Enable/Señal de activacion de transmision) y CSN (chip-select SPI). */
...
/* Funciones de manejo de spi en spi_g2955.c */
/* . */
uint8_t r_reg(uint8_t addr)
{
	...
	CSN_EN;
	i = spi_transferir16(RF24_NOP | ((addr & RF24_REGISTER_MASK) << 8));
...
void w_reg(uint8_t addr, uint8_t data)
{
	uint16_t i;
	CSN_EN;
	i = spi_transferir16( (data & 0x00FF) | (((addr & RF24_REGISTER_MASK) | RF24_W_REGISTER) << 8) );
	rf_status = (uint8_t) ((i & 0xFF00) >> 8);
	CSN_DIS;
}
...
void w_tx_addr(uint8_t *addr)
{
	...
	CSN_EN;
	rf_status = spi_transferir(RF24_TX_ADDR | RF24_W_REGISTER);
	for (i=rf_addr_width-1; i>=0; i--) {
		spi_transferir(addr[i]);
	}
	CSN_DIS;
}
...
void w_rx_addr(uint8_t pipe, uint8_t *addr)
{
	...
	CSN_EN;
	rf_status = spi_transferir((RF24_RX_ADDR_P0 + pipe) | RF24_W_REGISTER);
	if (pipe > 1) {  // Pipes 2-5 differ from pipe1's addr only in the LSB.
		spi_transferir(addr[rf_addr_width-1]);
	} else {
		for (i=rf_addr_width-1; i>=0; i--) {
			spi_transferir(addr[i]);
		}
...
void w_tx_payload(uint8_t len, uint8_t *data)
{
	...
	if (len % 2) {  // Odd payload size?  Make it even by stuffing the command in a 16-bit xfer
		...
		i = spi_transferir16( (RF24_W_TX_PAYLOAD << 8) | (0x00FF & data[0]) );
		rf_status = (i & 0xFF00) >> 8;
		i = 1;
	} else {
		rf_status = spi_transferir(RF24_W_TX_PAYLOAD);
	}
	for (; i < len; i+=2) {
		// SPI transfers MSB first
		spi_transferir16( (data[i] << 8) | (0x00FF & data[i+1]) );
	}
	CSN_DIS;
}
...
void w_tx_payload_noack(uint8_t len, uint8_t *data)
{
	...
	if (len % 2) {
		// Borrowing 'i' to extract STATUS...
		i = spi_transferir16( (RF24_W_TX_PAYLOAD_NOACK << 8) | (0x00FF & data[0]) );
		rf_status = (i & 0xFF00) >> 8;
		i = 1;
	} else {
		rf_status = spi_transferir(RF24_W_TX_PAYLOAD_NOACK);
	}
	for (; i < len; i+=2) {
		// SPI transfers MSB first
		spi_transferir16( (data[i] << 8) | (0x00FF & data[i+1]) );
...
uint8_t r_rx_peek_payload_size()
{
	...
	CSN_EN;
	i = spi_transferir16(RF24_NOP | (RF24_R_RX_PL_WID << 8));
	rf_status = (uint8_t) ((i & 0xFF00) >> 8);
	CSN_DIS;
	return (uint8_t) (i & 0x00FF);
}
...
uint8_t r_rx_payload(uint8_t len, uint8_t *data)
{
	...
	if (len % 2) {
		// Borrowing 'i' to extract STATUS...
		i = spi_transferir16((RF24_R_RX_PAYLOAD << 8) | RF24_NOP);
		rf_status = (i & 0xFF00) >> 8;
		data[0] = i & 0x00FF;
		i = 1;
	} else {
		rf_status = spi_transferir(RF24_R_RX_PAYLOAD);
	}
	for (; i < len; i+=2) {
		j = spi_transferir16(0xFFFF);
		// SPI transfers MSB first
		data[i] = (j & 0xFF00) >> 8;
		data[i+1] = (j & 0x00FF);
	}
...
{
	CSN_EN;
	rf_status = spi_transferir(RF24_FLUSH_TX);
	CSN_DIS;
}
...
{
	CSN_EN;
	rf_status = spi_transferir(RF24_FLUSH_RX);
	CSN_DIS;
}
...
{
	CSN_EN;
	rf_status = spi_transferir(RF24_REUSE_TX_PL);
	CSN_DIS;
}
...
void w_ack_payload(uint8_t pipe, uint8_t len, uint8_t *data)
{
	...
	if (len % 2) {
		// Borrowing 'i' to extract STATUS...
		i = spi_transferir16( ((RF24_W_ACK_PAYLOAD | pipe) << 8) | (0x00FF & data[0]) );
		rf_status = (i & 0xFF00) >> 8;
		i = 1;
	} else {
		rf_status = spi_transferir(RF24_W_ACK_PAYLOAD | pipe);
	}
	for (; i < len; i+=2) {
		// SPI transfers MSB first
		spi_transferir16( (data[i] << 8) | (0x00FF & data[i+1]) );
...
/* Parametros de configuracion de RF */
uint8_t rf_crc;
uint8_t rf_addr_width;
uint8_t rf_speed_power;
uint8_t rf_channel;
/* Variable de Status actualizada cada vez que se utiliza SPI */
uint8_t rf_status;
/* El estado de las interrupciones en el módulo es almacenado en esta variable despues de llamar a msprf24_get_irq_reason(). Si se produjo una interrupción, rf_irq tiene que tener el bit RF24_IRQ_FLAGGED seteado.*/
...
/* Inicialización y configuración del nrf24l01+ 
 * Inicializacion SPI
 * Inicializacion de puertos y pines
 */
void msprf24_init()
{
	/*Variables de configuraciones con los valores deseados MOVER A OTRO LADO*/
	...
	// Setup SPI
	spi_inicializar();
	// _EINT();  // Habilitar interrupciones (set GIE in SR)
	...
	// Setup IRQ
		...
		P4REN |= nrfIRQpin;
//		P4IES |= nrfIRQpin;   // Trigger on falling-edge No para el 955
//		P4IFG &= ~nrfIRQpin;  // Clear any outstanding IRQ
//		P4IE |= nrfIRQpin;    // Enable IRQ interrupt
	#endif

	// Conf. puertos de CSN/CE
	#if nrfCSNport == 1
		P1DIR |= nrfCSNpin;
	#elif nrfCSNport == 2
		P2DIR |= nrfCSNpin;
	...
	/* transferencia spi vacia con el CS deshabilitado para evitar el bug USI5 en el MSP430G2452 */
	spi_transferir(RF24_NOP);

	// Espera de 100ms para que se inicialice el transreceptor.
	uint8_t c = 20;
	for (; c; c--) {
		__delay_cycles(DELAY_CYCLES_5MS);
	}

	// Configurar el nrf con los valores de las variables de configuracion actuales
	msprf24_irq_clear(RF24_IRQ_MASK);  // Ignorar cualquier IRQ pendiente
	msprf24_close_pipe_all();          // Todos los pipes cerrados
					    
       
	msprf24_set_retransmit_delay(2000);  // A discreción
	msprf24_set_retransmit_count(15);    // A discreción
	msprf24_set_speed_power();
	msprf24_set_channel();
	msprf24_set_address_width();
	rf_feature = 0x00;  // Todas la "features" deshabilitadas
	msprf24_enable_feature(RF24_EN_DPL);      // Tamaño de payload dinamico
	msprf24_enable_feature(RF24_EN_DYN_ACK);  // Capacidad de usar w_tx_payload_noack()
...
/* Funciones para des/habilitar caracteristicas del nrf */
...
{
	CSN_EN;
	rf_status = spi_transferir(RF24_NOP);
	CSN_DIS;
...
void msprf24_irq_clear(uint8_t irqflag)
{
	...
	CSN_EN;
	rf_status = spi_transferir(RF24_STATUS | RF24_W_REGISTER);
	spi_transferir(irqflag);
	CSN_DIS;

	// Per datasheet procedure, check FIFO_STATUS to see if there's more RX FIFO data to process.
	if (irqflag & RF24_IRQ_RX) {
		CSN_EN;
		rf_status = spi_transferir(RF24_FIFO_STATUS | RF24_R_REGISTER);
		fifostat = spi_transferir(RF24_NOP);
		CSN_DIS;
...
// Manejo de las interrupciones que genera el nrf. NO DISPONIBLE PARA LA PLACA EN LA PLACA DEL RECEPTOR.
#if   nrfIRQport == 2
  #ifdef __GNUC__
